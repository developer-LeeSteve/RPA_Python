...
excel_data = pd.read_excel("C:\\Program Files\\Automation Anywhere\\Bot Agent\\tesnile test\\stress_strain1.xlsx")
...
for i in range(n_samples):
   if strain_list[i] > 0.02:
      ...
      g_sample = _gradient(strain_list[i], strain_list[i+10], stress_list[i], stress_list[i+10])
      if g_sample <= 0:
         continue
      else:
...
#    print(_intersection())
   ...
   # plt.plot(intersection[0], intersection[1], marker='o', markersize=5)
   # plt.show()